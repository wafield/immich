...
import type { JobItem, JobOf, StorageAsset } from 'src/types.js';
...
import { getLivePhotoMotionFilename } from 'src/utils/file.js';
  ...
  async moveLibrary(
    ...
    for (const assetId of dto.assetIds) {
      try {
        ...
        const result = await storageTemplateService.moveAssetToLibrary({
          assetId,
          targetLibraryId: dto.targetLibraryId ?? null,
          targetUploadPath: rootPath,
        });
